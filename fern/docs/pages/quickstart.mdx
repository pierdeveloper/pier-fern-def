...
(async () => {
    ...
    // Instantiate each api needed
    const borrowersApi = new BorrowersApi(new Configuration({
        basePath: PIER_API_BASE_PATH,
        // Can set token here but need to instantiate a new client when auth runs out in 24h
        // accessToken: token
    }));
    ...
    const {data: borrowers} = await borrowersApi.borrowersListAllBorrowers({
        headers: {
            // Can set the token each request and update it regularly to not have to worry about expiration
